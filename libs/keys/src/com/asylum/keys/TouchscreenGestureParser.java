...
import static com.android.internal.R.styleable.TouchscreenGesture;
import static com.android.internal.R.styleable.TouchscreenGesture_name;
import static com.android.internal.R.styleable.TouchscreenGesture_path;
import static com.android.internal.R.styleable.TouchscreenGesture_scancode;
import static com.android.internal.R.styleable.TouchscreenGesture_defaultAction;
...
public class TouchscreenGestureParser {
    ...
    public static GesturesArray parseGestures(Context context) {
        ...
        try {
            ...
            parser = res.getXml(com.android.internal.R.xml.touchscreen_gestures);
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                if ("gesture".equals(nodeName)) {
                    TypedArray sa = res.obtainAttributes(attrs, TouchscreenGesture);

                    Gesture g = new Gesture();
                    TypedValue tv = sa.peekValue(TouchscreenGesture_name);
                    if (tv != null && tv.type == TypedValue.TYPE_STRING) {
                        if (tv.resourceId != 0) {
                            g.name = res.getString(tv.resourceId);
                        } else {
                    ...
                    if (tv != null && tv.type == TypedValue.TYPE_STRING) {
                        if (tv.resourceId != 0) {
                            g.path = res.getString(tv.resourceId);
                        } else {
                    ...
                    if (tv != null && tv.type == TypedValue.TYPE_STRING) {
                        if (tv.resourceId != 0) {
                            g.def = res.getString(tv.resourceId);
                        } else {
                    ...
                    if (tv != null && tv.type == TypedValue.TYPE_INT_DEC) {
                        if (tv.resourceId != 0) {
                            g.scancode = res.getInteger(tv.resourceId);
                        } else {
                            g.scancode = tv.data;
                        }
                    }
